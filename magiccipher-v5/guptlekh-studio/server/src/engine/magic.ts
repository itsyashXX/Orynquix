...
import type { AnalysisCandidate, AnalysisResponse, Confidence, RecipeStep, SearchMode, XrayStep } from '../../../shared/types.js';
...
export function analyzeMagic(input: string, options: number | AnalyzeOptions = 3): AnalysisResponse {
  ...
  const budget = searchMode === 'fast' ? 1800 : searchMode === 'deep' ? 12000 : 5000;
  ...
  xray.push({stage:'Search',message:`Tested ${tested} candidate transformations up to depth ${maxDepth} in ${searchMode} mode; Vigenère ${suppliedKey ? 'key-assisted' : 'speculation enabled'}`,status:'info'});
  ...
  return {
    ...
    xray,
    meta:{elapsedMs:Number((performance.now()-started).toFixed(1)),candidatesTested:tested,recursionDepth:maxDepth,engineVersion:'5.0.0',keyUsed:Boolean(suppliedKey),searchMode,stoppedBecause: tested >= budget ? 'search budget reached' : queue.length ? 'depth limit reached' : 'candidate space exhausted'}
  };
}
