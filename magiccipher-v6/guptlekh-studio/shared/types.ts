...
export interface FamilyEstimate { family:string; score:number; reason:string; }
export type SearchMode = 'fast'|'balanced'|'deep';
export interface AnalysisResponse {
  input:string; normalizedLength:number; entropy:number; likelyCategory:string;
  best:AnalysisCandidate|null; alternatives:AnalysisCandidate[]; xray:XrayStep[]; families:FamilyEstimate[];
  meta:{ elapsedMs:number; candidatesTested:number; recursionDepth:number; engineVersion:string; keyUsed?:boolean; searchMode?:SearchMode; stoppedBecause?:string; reliable?:boolean; searchStage?:string; };
}
