...
const router=Router();
const schema=z.object({input:z.string().min(1).max(50000),mode:z.enum(['encrypt','decrypt']),algorithm:z.enum(['vigenere','caesar','rot13','atbash','base64','hex','xor','aes-gcm']),key:z.string().max(512).optional(),shift:z.number().int().min(-1000).max(1000).optional()});
