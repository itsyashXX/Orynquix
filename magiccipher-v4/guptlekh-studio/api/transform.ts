...
export default function handler(req:VercelRequest,res:VercelResponse){
  ...
  try{
    ...
    if(!['encrypt','decrypt'].includes(mode)||!['vigenere','caesar','rot13','atbash','base64','hex','xor','aes-gcm'].includes(algorithm)) return res.status(400).json({error:'Invalid transform request'});
